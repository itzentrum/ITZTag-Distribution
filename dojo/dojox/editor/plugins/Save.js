...
//			"Content-Type": "text/html"
		...
		if(this.url){
//		    var norm=content.replace(/\/\*/g,"C_STYLE_COMMENT_BEGIN").replace(/\*\//g,"C_STYLE_COMMENT_END").replace(/<!--/g,"HTML_STYLE_COMMENT_BEGIN").replace(/-->/g,"HTML_STYLE_COMMENT_END").replace(/"/g,"\\\"").replace(/\n+/g,'\n').replace(/\s+/g," ");
		    var norm=escape(encodeURI(content.replace(/\n+/g,'\n').replace(/\s+/g," ").replace(/"/g,"\\\"")));
		    var postArgs = {
			url: this.url,
			//			        postData: content,
